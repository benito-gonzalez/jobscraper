...
def generate_instance_from_client(client_name, url):
    ...
    if client_name == "Curious AI":
        return CuriousAI(client_name, url)
    if client_name == "DAIN Studios":
        return DAINStudios(client_name, url)
    if client_name == "Fourkind":
        return Fourkind(client_name, url)
    if client_name == "Inscripta":
...
class Tieto(Scraper):
            ...
            while not last_page:
                ...
                for item in job_div.find_all('a'):
                    if "jobResultRow" in item.get('class'):
                        ...
                        if self.is_valid_job(title, description_url, description):
                            location = self.get_location(item, title)
                            end_date = self.get_dates(item, title)
    ...
    def get_mandatory_fields(self, item):
        ...
        title_tag = item.find('div', {'class': 'col-md-3'})
        if title_tag:
            title = title_tag.text.strip()
    ...
    def get_description(url):
        description = ""
        job_details_html = request_support.simple_get(url, accept_json=True)
        if job_details_html:
            json_dict = json.loads(job_details_html)
            for child in json_dict["body"]["children"][1]["children"][0]["children"]:
                if child["ecid"] == "richTextArea.jobPosting.jobDescription":
    ...
    def get_location(self, item, title):
        # third div contains the location
        div_list = item.find_all('div', {'class': 'col-md-3'})
        location = None

        if len(div_list) == 4:
            location = div_list[2].text.strip()
    ...
    def get_dates(self, item, title):
        # last div contains the date
        div_list = item.find_all('div', {'class': 'col-md-3'})
        last_div = end_date = None

        for last_div in div_list:
            pass
        if last_div:
            end_date_datetime = parser.parse(last_div.text.strip())
            end_date = end_date_datetime.strftime('%Y-%m-%d')
...
class FuturePlay(Scraper):
    ...
    def extract_info(self, html):
        ...
        jobs = []
        soup = BeautifulSoup(html, 'html.parser')
        for job_div in soup.find_all('li', class_="at-jobs-list-item"):
            if job_div.find('a'):
                title, description_url, description = self.get_mandatory_fields(job_div)
                if self.is_valid_job(title, description_url, description):
                    location = self.get_location(job_div, title)
                    end_date = self.get_end_date(job_div, title)
    ...
    def get_full_description(url):
        ...
        if job_details_html:
            job_details_soup = BeautifulSoup(job_details_html, 'html.parser')
            details_block = job_details_soup.find('div', class_='at-job-body')
            if details_block:
                for child in details_block.children:
                    if isinstance(child, Tag):
                        if child.name == "img":
    ...
    def get_location(self, item, title):
        ...
        containers = item.find_all('div', class_='at-jobs-list-cell-40')
        if len(containers) == 3:
            location = containers[1].get_text().strip()
...
class RedhillGames(Scraper):
    ...
    def extract_info(self, html):
        ...
        jobs = []
        soup = BeautifulSoup(html, 'html.parser')
        block = soup.find('div', class_='job-toggles-container')
        if block:
            for item in block.find_all('div', class_='job-toggle'):
                title, description_url, description = self.get_mandatory_fields(item)
                if self.is_valid_job(title, description_url, description):
    ...
    def get_full_description(url):
        ...
        if job_details_html:
            job_details_soup = BeautifulSoup(job_details_html, 'html.parser')
            divs = job_details_soup.find_all('div', class_='entry-content')
            for div in divs:
                if "two-images grid" in div.get('class'):
                    continue
...
class Matchmade(Scraper):
    ...
    def extract_info(self, html):
        ...
        if "offers" in json_dict:
            for item in json_dict["offers"]:
                title, description_url, description = self.get_mandatory_fields(item)
                if self.is_valid_job(title, description_url, description):
                    if "city" in item:
                        location = item["city"]
                    ...
                    if "employment_type_code" in item:
                        job_type = item["employment_type_code"]
                    else:
                        job_type = None
    ...
    def get_mandatory_fields(self, item):
        ...
        if "title" in item:
            title = item["title"]
            if "careers_url" in item:
                description_url = item['careers_url']
                description = self.get_description(item)
    ...
    def get_description(item):
        ...
        if "requirements" in item:
            ...
            soup = BeautifulSoup(requirements_raw, 'html.parser')
            for child in soup.children:
                Scraper.clean_attrs(child)
                description += str(child)
...
class Frosmo(Scraper):
    ...
    def extract_info(self, html):
        ...
        for item in soup.find_all('h2', class_='elementor-heading-title elementor-size-default'):
            if item.find('a'):
                title, description_url, description = self.get_mandatory_fields(item)
                if self.is_valid_job(title, description_url, description):
                    location = self.get_location(item, title)
    ...
    def get_location(self, item, title):
        ...
        parent = item.find_parent('div', class_='elementor-element')
        if parent:
            sibling = parent.find_next_sibling('div')
            if sibling:
                location_tag = sibling.find('h3', class_='elementor-icon-box-title')
                if location_tag:
                    location = location_tag.get_text()
                    if "Finland" in location:
                        # it has office in different countries but if it is Finland, it is based in Helsinki
                        location = "Helsinki"
...
class Iceye(Scraper):
    ...
    def extract_info(self, html):
        # POST request to https://careers-page.workable.com/api/v1/accounts/iceye/jobs
        log_support.log_extract_info(self.client_name)
        jobs = []

        post_url = "https://careers-page.workable.com/api/v1/accounts/iceye/jobs"
        body = {"query": "", "location": [{"country": "Finland", "region": "Uusimaa", "city": "Espoo"}], "department": [], "worktype": [], "remote": []}
    ...
    def get_mandatory_fields(self, item):
        ...
        if "title" in item:
            title = item["title"]
        if "shortcode" in item:
            description_url = "https://apply.workable.com/iceye/j/" + item["shortcode"]
        if "description" in item:
            description = item["description"]
...
class Ul(Scraper):
    ...
    def extract_info(self, html):
        ...
        if open_positions:
            for item in open_positions.find_next_siblings('p'):
                title, description_url, description, location = self.get_mandatory_fields(item)
                if self.is_valid_job(title, description_url, description):
    ...
    def get_description(description_url):
        ...
        if job_details_html:
            ...
            container = soup.find('div', class_='mt20')
            if container:
                for child in container.children:
                    if isinstance(child, Tag):
                        Scraper.clean_attrs(child)
            ...
            location_tag = soup.find('p', class_='accent')
            if location_tag:
                location = location_tag.get_text().strip()
...
class DearLucy(Scraper):
    ...
    def extract_info(self, html):
        ...
        jobs_div = soup.find('div', {'id': 'content'})
        if jobs_div:
            for item in jobs_div.find_all('div', class_='col sqs-col-7 span-7'):
                title, description_url, description = self.get_mandatory_fields(item)
                if self.is_valid_job(title, description_url, description):
                    location = "Helsinki"
    ...
    def get_mandatory_fields(self, item):
        ...
        if title_tag:
            title = title_tag.get_text().strip()
            if title.lower().endswith(', helsinki'):
                title = title.rsplit(',', 1)[0]

            url_tag = item.find('a')
            if url_tag:
                relative_url = url_tag.get('href')
                description_url = self.url.split(".co/")[0] + ".co" + relative_url
                if description_url:
                    description = self.get_description(description_url)
    ...
    def get_description(url):
        ...
        if job_details_html:
            ...
            if parent_div:
                for container in parent_div.find_all('div', class_='sqs-block-content'):
                    # first container with at least one <p> will have the job description
                    if container.find('p'):
                        for child in container.children:
                            if isinstance(child, Tag):
                                Scraper.clean_attrs(child)
...
class Sellforte(Scraper):
    ...
    def extract_info(self, html):
        ...
        for item in soup.find_all('div', class_='hs-button'):
            title, description_url, description = self.get_mandatory_fields(item)
            if self.is_valid_job(title, description_url, description):
    ...
    def get_description(url, div_id):
        ...
        if job_details_html:
            ...
            if container:
                description_block = container.find('div', class_='hs-middle-content')
                for child in description_block.children:
                    if isinstance(child, Tag):
                        Scraper.clean_attrs(child)
                        if child.name == "img":
...
class CuriousAI(Scraper):
    ...
    def extract_info(self, html):
        ...
        container = soup.find('div', class_='post-type-job')
        if container:
...
class DAINStudios(Scraper):
    ...
    def get_mandatory_fields(self, item):
        ...
        title_tag = item.find('h3', class_='t-entry-title')
        if title_tag:
            title = title_tag.get_text()
            url_tag = title_tag.find('a')
            if url_tag:
    ...
    def get_description(url):
        ...
        if job_details_html:
            soup = BeautifulSoup(job_details_html, 'lxml')
            container = soup.find('div', class_='post-body')
            if container:
                for child in container.children:
                    if isinstance(child, Tag):
                        Scraper.clean_attrs(child)
...
class Fourkind(Scraper):
    ...
    def extract_info(self, html):
        ...
        for item in soup.find_all("div", class_='mkdf-elements-holder'):
            # it needs to skip the item for "load more" button
            if not item.find('a', class_='sbi_load_btn'):
                title, description_url, description = self.get_mandatory_fields(item)
                if self.is_valid_job(title, description_url, description):
                    location = self.get_location(item, title)
    ...
    def get_description(url):
        ...
        if job_details_html:
            soup = BeautifulSoup(job_details_html, 'lxml')
            container = soup.find('div', class_=lambda value: value and "wpb_text_column wpb_content_element vc_custom_" in value)
            if container:
                for child in container.children:
                    if isinstance(child, Tag):
                        Scraper.clean_attrs(child)
    ...
    def get_location(self, item, title):
        ...
        location_tag = item.find('strong')
        if location_tag:
            if location_tag.next_sibling and location_tag.next_sibling.next_sibling:
                location = location_tag.next_sibling.next_sibling.strip()
...
class Inscripta(Scraper):
    ...
    def get_mandatory_fields(self, item):
        ...
        title_tag = item.find('h5', class_='job-title')
        if title_tag:
            title = title_tag.get_text().strip()
            url_tag = item.find('a')
            if url_tag:
    ...
    def get_description(url):
        ...
        if job_details_html:
            soup = BeautifulSoup(job_details_html, 'lxml')
            for container in soup.find_all('div', class_='description'):
                for child in container.children:
                    if isinstance(child, Tag):
                        if child.get_text().startswith("Keywords:") or child.get_text().startswith("Key words:"):
                            continue
                        Scraper.clean_attrs(child)
                        if child.get_text().strip() != "":
                            description += str(child)
    ...
    def get_location(self, item, title):
        ...
        location_tag = item.find('li', class_='job-location')
        if location_tag:
            location = location_tag.get_text().strip()
...
class Speechgrinder(Scraper):
    ...
    def extract_info(self, html):
        ...
        for item in soup.find_all("div", class_='role-item'):
            title, description_url, description = self.get_mandatory_fields(item)
            if self.is_valid_job(title, description_url, description):
                location = self.get_location(item, title)
    ...
    def get_mandatory_fields(self, item):
        ...
        if title_tag:
            title = title_tag.get_text().strip()
            url_tag = item.find('a', class_='aply-btn')
            if url_tag:
                relative_url = url_tag.get('href')
                if relative_url:
                    description_url = self.url.split(".com/")[0] + ".com" + relative_url
    ...
    def get_location(self, item, title):
        ...
        location_tag = item.find('span')
        if location_tag:
            location = location_tag.next_sibling
